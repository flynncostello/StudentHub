...
const groupChatroomsAPI = {
  ...
  getGroupChatroomMessages: async (chatroom_id) => {
    try {
      const response = await axios.get(`${API_ENDPOINT}/group_chatrooms/${chatroom_id}/messages`);
      return response.data;
    } catch (error) {
      console.error('Error getting group chatroom messages:', error);
      throw error;
    }
  },
  createGroupChatroomMessage: async (chatroom_id, message_info) => {
    try {
      const response = await axios.post(`${API_ENDPOINT}/group_chatrooms/${chatroom_id}/messages`, message_info);
      return response.data;
    } catch (error) {
      console.error('Error creating group chatroom message:', error);
      throw error;
    }
  }
};
